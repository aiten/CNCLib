...
void CMenuBase::MenuButtonPressSetMenu(const SMenuItemDef*def)
{
	...
	if (posMenu!=NULL)
	{
		...
		_position = newMenu->FindMenuIdx((intptr_t) def, [](const SMenuItemDef* def, intptr_t param) -> bool
		{
...
void CMenuBase::MenuButtonPressMenuBack(const SMenuItemDef* def)
{
	...
	SetPosition(GetMenuDef()->FindMenuIdx((intptr_t) oldMenu, [](const SMenuItemDef* def, intptr_t oldMenu) -> bool
	{
