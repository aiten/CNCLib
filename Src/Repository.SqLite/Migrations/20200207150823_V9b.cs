        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateTable(
                ...
                columns: table => new
                {
                    ...
                    UserId = table.Column<int>(nullable: false),
                    Name = table.Column<string>(maxLength: 64, nullable: false),
                    SerialServer = table.Column<string>(maxLength: 128, nullable: true),
                    SerialServerUser = table.Column<string>(maxLength: 32, nullable: true),
                    SerialServerPassword = table.Column<string>(maxLength: 64, nullable: true),
                    ...
                    WorkOffsets = table.Column<int>(nullable: false)
                },
                constraints: table =>
                {
            ...
            migrationBuilder.Sql("insert into User (UserId,Name,Password)"+
                                 " select           UserId,Name,Password"+
                                 " from User_backup");

            migrationBuilder.Sql("insert into Configuration ([ConfigurationId],[UserId],[Group],[Name],[Type],[Value])"+
                                 " select                    [ConfigurationId],[UserId],[Group],[Name],[Type],[Value]"+
                                 " from Configuration_backup");
            
            migrationBuilder.Sql("insert into UserFile (UserFileId,UserId,FileName,Content)"+
                                 " select               UserFileId,UserId,FileName,Content"+
                                 " from UserFile_backup");

            migrationBuilder.Sql("insert into Item (ItemId,UserId,Name,ClassName)"+
                                 " select           ItemId,UserId,Name,ClassName"+
                                 " from Item_backup");

            migrationBuilder.Sql("insert into ItemProperty ([ItemId],[Name],[Value])"+
                                 " select                   [ItemId],[Name],[Value]"+
                                 " from ItemProperty_backup");

            migrationBuilder.Sql("insert into Machine ([MachineId],[Axis],[BaudRate],[BufferSize],[ComPort],[CommandSyntax],[CommandToUpper],[Coolant],[Laser],[Name],[ProbeDist],[ProbeDistUp],[ProbeFeed],[ProbeSizeX],[ProbeSizeY],[ProbeSizeZ],[Rotate],[SDSupport],[SizeA],[SizeB],[SizeC],[SizeX],[SizeY],[SizeZ],[Spindle],[UserId],[DtrIsReset],[SerialServer],[SerialServerPassword],[SerialServerUser],[WorkOffsets])"+
                                 " select              [MachineId],[Axis],[BaudRate],[BufferSize],[ComPort],[CommandSyntax],[CommandToUpper],[Coolant],[Laser],[Name],[ProbeDist],[ProbeDistUp],[ProbeFeed],[ProbeSizeX],[ProbeSizeY],[ProbeSizeZ],[Rotate],[SDSupport],[SizeA],[SizeB],[SizeC],[SizeX],[SizeY],[SizeZ],[Spindle],[UserId],[DtrIsReset],[SerialServer],[SerialServerPassword],[SerialServerUser],[WorkOffsets]"+
                                 " from Machine_backup");
            
            migrationBuilder.Sql("insert into MachineCommand ([MachineCommandId],[CommandName],[CommandString],[MachineId],[PosX],[PosY],[JoystickMessage])"+
                                 " select                     [MachineCommandId],[CommandName],[CommandString],[MachineId],[PosX],[PosY],[JoystickMessage]"+
                                 " from MachineCommand_backup");

            migrationBuilder.Sql("insert into MachineInitCommand ([MachineInitCommandId],[SeqNo],[CommandString],[MachineId])"+
                                 " select                         [MachineInitCommandId],[SeqNo],[CommandString],[MachineId]"+
                                 " from MachineInitCommand_backup");

            migrationBuilder.Sql("insert into Log ([Id],[LogDate],[Application],[Level],[Message],[UserName],[ServerName],[MachineName],[Port],[Url],[ServerAddress],[RemoteAddress],[Logger],[Exception],[StackTrace])" +
                                 " select          [Id],[LogDate],[Application],[Level],[Message],[UserName],[ServerName],[MachineName],[Port],[Url],[ServerAddress],[RemoteAddress],[Logger],[Exception],[StackTrace]" +
                                 " from Log_backup");
            ...
            migrationBuilder.Sql("drop table Log_backup");
        }
